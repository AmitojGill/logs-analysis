...
def report_author_views(author_view_list):
	...
	'''
	Objective of report_author_views() function is to print top authors and their total views
...
def get_most_error_per_day():
	...
	'''
	Objective of get_most_error_per_day() function is to get most error rate per day
	INPUTS: There are no inputs to this function
	OUTPUTS: it returns a list of any days that has more than 1% error rate
	VIEWS IN PostgreSQL: error_per_day, requests_per_day, and error_rate_per_day views must be created, 
	...
	error_rate = '''SELECT * FROM error_rate_per_day WHERE percentage_error > 1;'''
...
def report_most_error_per_day(most_error):
	'''
	Objective of report_most_error_per_day() function is to print days and error rate 
	with more than 1% error rate per day
	INPUTS: most_error is an input in a list format with tuples as values for the list
	...
	for days in most_error:
		print(str(days[0]) + " - " + str((format(days[1],'.2f'))) + "%  errors")
